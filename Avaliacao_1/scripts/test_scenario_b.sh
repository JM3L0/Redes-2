...
# Formato de saida customizado do cURL
CURL_FORMAT="%{time_namelookup};%{time_connect};%{time_appconnect};%{time_starttransfer};%{time_total};%{speed_download};%{size_download}\n"

for FILE in "${FILES[@]}"; do
    FILE_LABEL=$(echo "${FILE}" | sed 's/\.bin//')
    ...
    for PROTO in "${PROTOCOLS[@]}"; do
        ...
        for i in $(seq 1 ${REPETITIONS}); do
            # Dispara download descartando payload em /dev/null
            RES=$(curl -k -s -o /dev/null -w "${CURL_FORMAT}" ${PROTO} "${SERVER_URL}/${FILE}")
            
            IFS=';' read -r DNS CONN TLS TTFB TOTAL SPEED SIZE <<< "${RES}"
            GOODPUT_MBPS=$(awk "BEGIN {print (${SPEED} * 8) / 1000000}")
