...
for SCENARIO in "${LOSS_SCENARIOS[@]}"; do
    ...
    for PROTO in "${PROTOCOLS[@]}"; do
        ...
        for i in $(seq 1 ${REPETITIONS}); do
            ...
            # Montar lista de URLs para os 100 objetos
            URL_LIST=()
            for obj in $(seq -w 1 100); do
                URL_LIST+=("${SERVER_URL}/obj_${obj}.bin")
            done

            # Baixar todos os 100 objetos, capturando bytes realmente transferidos por requisicao
            REAL_BYTES=$(curl -k -s -o /dev/null \
                -w "%{size_download}\n" ${PROTO} "${URL_LIST[@]}" 2>/dev/null \
                | awk '{s+=$1} END {print s+0}' || echo 0)

            END_TIME=$(date +%s.%N)
            BATCH_FCT=$(awk "BEGIN {printf \"%.4f\", ${END_TIME} - ${START_TIME}}")

            if [ "${REAL_BYTES}" -eq 0 ]; then
                echo "      [-] Repeticao ${i}/${REPETITIONS}: Nenhum byte recebido, descartando registro."
                continue
            fi
            ...
            echo "      -> Repeticao ${i}/${REPETITIONS}: Lote FCT=${BATCH_FCT}s, Bytes=${REAL_BYTES}, Goodput=${GOODPUT_MBPS} Mbps"
            sleep 0.5
        done
    done
done
